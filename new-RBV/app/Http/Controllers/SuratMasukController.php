...
class SuratMasukController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->filled('jenis')) {
            $data = $data->filter(fn ($s) => $s->jenis === $request->jenis);
        }
    ...
    public function store(Request $request)
    {
        ...
        $isUnit = in_array($user->role, ['unit', 'karyawan'])
            || $user->jabatan === 'kepala unit';

        if ($isUnit) {
            $request->merge([
                'jenis' => 'internal',
                'asal_surat' => $user->unit_kerja,
            ]);
        }
    ...
    public function show($id)
    {
        ...
        return view('pages.E-Office.SuratMasuk.suratmasuk_show', 'pages.KelolahAkun.tambah_akun', compact('surat', 'bisaApprove', 'jabatanApproval', 'unitsTerkait'));
    }
